#!/usr/bin/env python3
"""Patch grafana.yaml: replace the circleguard-overview dashboard ConfigMap JSON."""
import json, pathlib, textwrap

GRAFANA_YAML = pathlib.Path(__file__).parent.parent / "k8s/monitoring/grafana.yaml"
...
# Indent 4 spaces for YAML literal block
indented_json = "\n".join("    " + line for line in js.splitlines())

src = GRAFANA_YAML.read_text(encoding="utf-8")

# Find the start of the circleguard-overview.json key and the next --- separator
KEY = "  circleguard-overview.json: |"
sep_after = "\n\n---\n"

key_pos = src.index(KEY)
sep_pos = src.index(sep_after, key_pos)

new_block = f"{KEY}\n{indented_json}\n"
patched = src[:key_pos] + new_block + src[sep_pos:]

GRAFANA_YAML.write_text(patched, encoding="utf-8")
print(f"Patched {GRAFANA_YAML}")
print(f"New ConfigMap JSON: {len(js.splitlines())} lines, {len(js)} bytes")
